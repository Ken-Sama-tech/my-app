...
import type { Options } from "./AnimeOptions";
...
const searchEpisodes: SearchEpisodes = async (
  ...
  try {
    ...
    const activeExtensions: AnimeExtensions[] =
      extensions?.filter((ext) => ext.active).map((ext) => ext.extension) || [];
...
const AnimeExtensionResult: FC<AnimeExtensionResultProps> = ({ title, id }) => {
  ...
  const context = useContext(AnimeOptionsContext);
  const {
    data: search,
    isLoading,
    isError,
  ...
  const [logoCollection] = useState<Map<AnimeExtensions, string>>(
    new Map(
      context?.current?.extensions?.map((ext) => [ext.extension, ext.logo])
    )
  );
  ...
  return (
    <section className="bg-neutral-900 w-full h-fit px-2 relative py-5">
      {!isLoading && (
        <div className="w-full min-h-60 h-full bg-neutral-800 rounded-2xl">
          {!isError && (
            <div className="size-full flex relative p-3 gap-2 flex-wrap justify-center min-h-60 h-full md:justify-start">
              {!search && (
              ...
              {search?.map((res) => {
                return (
                  <div
                    key={`${res.extension}-${res.data?.title}`}
                  ...
                  >
                    <div className="flex items-center w-full gap-0.5">
                      <div
                        className="aspect-square rounded-md h-[30px] bg-no-repeat bg-cover bg-center"
                        style={{
                    ...
                    <div className="w-full grow justify-center items-center shrink-0 flex gap-1">
                      ...
                      {!res.error && (
                        <>
                          <div className="w-1/2">
                            <button
                              disabled={!res.data?.episodes.dub}
                          ...
                          <div className="w-1/2">
                            <button
                              disabled={!res.data?.episodes.sub}
      ...
      {isLoading && (
        <div className="w-full h-60 rounded-2xl z-1 skeleton bg-neutral-800"></div>
      )}
    </section>
  );
};
